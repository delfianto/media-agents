...
| Skill | Description |
| :--- | :--- |
| [**`av1-transcode`**](skills/av1-transcode) | Re-encodes video to AV1 (CPU via `libsvtav1`, or GPU via `av1_nvenc` on an Ada Lovelace+ NVIDIA GPU) and audio to Opus, squeezing max quality per byte out of Blu-ray remuxes -- resolution/HDR/Dolby-Vision-aware presets for film and anime/cartoon sources, live progress monitoring, and the same dry-run/verify-then-swap safety model as `media-library`. |
