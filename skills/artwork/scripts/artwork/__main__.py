#!/usr/bin/env python3
"""Entrypoint: python3 artwork/__main__.py ...

Lives inside the `artwork` package itself (rather than as a
...
`python -m artwork` works from `scripts/` if that's ever useful. The
...
# __main__.py lives inside artwork/ itself, one level below scripts/
# -- that's the directory that needs to be on sys.path for `import
# artwork` to resolve (and for `.` in the package's own submodules
# to work).
