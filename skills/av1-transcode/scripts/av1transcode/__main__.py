#!/usr/bin/env python3
"""Entrypoint: python3 av1transcode/__main__.py <probe|list-presets|run|purge-backups> ...

Lives inside the `av1transcode` package itself (rather than as a same-named
...
the package's own name), and it also means `python -m av1transcode` works
from `scripts/` if that's ever useful.
...
scripts/av1transcode/__main__.py ...`. `.agents` is commonly a symlink into
the real checkout elsewhere; see _find_own_path's docstring for why a
relative invocation needs care to avoid losing that symlink's name.
"""
...
# __main__.py lives inside av1transcode/ itself, one level below scripts/ --
# that's the directory that needs to be on sys.path for `import av1transcode`
