...
Every other skill here assumes a handful of external binaries, Python packages, and (for `transcode`'s GPU path) specific hardware are already present on the machine it runs on -- and each one fails in its own way, at its own time, when one of those is missing (an `transcode` run that dies partway through the first file because `nvencc` isn't installed, an `organize` run that errors on a missing `TMDB_API_KEY`, `mkvedit` failing without `mkvpropedit`, or `trackstrip apply` failing without `mkvmerge`). This skill checks all of it up front, in one pass, before any of that.
...
```bash
.agents/run.sh env-check --category transcode   # only that skill's checks
.agents/run.sh env-check --required-only             # hide optional/nice-to-have checks
```
...
| `shared` | `ffmpeg`, `ffprobe` | Yes -- both `track-strip` and `transcode` shell out to these directly. |
