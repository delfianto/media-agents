#!/usr/bin/env python3
"""Entrypoint: python3 envcheck/__main__.py [--category NAME] [--required-only]

Lives inside the `envcheck` package itself (rather than as a same-named
...
the package's own name), and it also means `python -m envcheck` works from
...
# __main__.py lives inside envcheck/ itself, one level below scripts/ --
# that's the directory that needs to be on sys.path for `import envcheck`
