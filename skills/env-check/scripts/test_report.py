...
from envcheck.checks import CheckResult
from envcheck.report import (
    exit_code,
    format_report,
    group_by_category,
    missing_optional,
...
def test_missing_required_only_returns_required_and_missing():
    results = [
        _result("ffmpeg", "shared", True, True),
        _result("nvencc", "av1-transcode", False, False),
...
def test_format_report_marks_missing_optional_differently_from_required():
    results = [_result("nvencc", "av1-transcode", False, False, install_hint="optional extra")]
    report = format_report(results)
    assert ".." in report
