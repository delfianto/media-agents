#!/usr/bin/env python3
"""Entrypoint: python3 mkvedit/__main__.py ...

Lives inside the `mkvedit` package itself (rather than as a
...
`python -m mkvedit` works from `scripts/` if that's ever useful. The
...
# __main__.py lives inside mkvedit/ itself, one level below scripts/
# -- that's the directory that needs to be on sys.path for `import
# mkvedit` to resolve (and for `.` in the package's own submodules
# to work).
