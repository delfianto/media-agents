...
# ///
"""Entrypoint: uv run organize/__main__.py ...
(or: pip install -r requirements.txt && python3 organize/__main__.py ...)

Lives inside the `organize` package itself (rather than as a
...
`python -m organize` works from `scripts/` if that's ever useful. The
...
# __main__.py lives inside organize/ itself, one level below scripts/
# -- that's the directory that needs to be on sys.path for `import
# organize` to resolve (and for `.` in the package's own submodules
# to work).
