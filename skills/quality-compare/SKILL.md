...
```bash
.agents/scripts/run-skill quality-compare \
...
.agents/scripts/run-skill quality-compare ... --mode quick

# Every frame; potentially very slow for a feature-length 4K source
.agents/scripts/run-skill quality-compare ... --mode full

# Persist the complete per-frame result
.agents/scripts/run-skill quality-compare ... --json-out comparison.json
```
