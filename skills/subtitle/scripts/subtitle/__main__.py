#!/usr/bin/env python3
"""Entrypoint: python3 subtitle/__main__.py ...

Lives inside the `subtitle` package itself (rather than as a
...
`python -m subtitle` works from `scripts/` if that's ever useful. The
...
# __main__.py lives inside subtitle/ itself, one level below scripts/
# -- that's the directory that needs to be on sys.path for `import
# subtitle` to resolve (and for `.` in the package's own submodules
# to work).
