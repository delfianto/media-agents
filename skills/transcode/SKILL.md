...
The implementation lives at `src/psammophis/transcode/` inside the packaged application. It wraps `ffprobe`/`ffmpeg` plus `mkvpropedit` for accurate output track-statistics tags, and for Dolby Vision / HDR10+ on GPU also `nvencc` (rigaya NVEnc with libdovi). See `reference/presets.md` for the full rationale (and citations) behind every encoder setting, and `reference/incidents.md` for real bugs a smoke test against actual files in this library caught before they ever became defaults - read both before changing `psammophis.medialib.av1_presets`, `command.py`, `nvencc_cmd.py`, or `psammophis.medialib.gpu`.
