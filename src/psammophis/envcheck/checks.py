...
from psammophis.medialib.gpu import detect_av1_nvenc_gpu, list_gpu_indices
...
def check_ffmpeg_encoder(encoder: str, *, required: bool, install_hint: str) -> CheckResult:
    ...
    if shutil.which("ffmpeg") is None:
        return CheckResult(name, "transcode", required, False, install_hint="install ffmpeg first")
    proc = _run("ffmpeg", "-hide_banner", "-h", f"encoder={encoder}")
    found = bool(proc and proc.returncode == 0 and "Encoder" in (proc.stdout or ""))
    return CheckResult(
        name, "transcode", required, found, install_hint="" if found else install_hint
    )
    ...
    return CheckResult(
        ...
        install_hint=(
            ...
            if found
                ...
                "svt-av1-hdr so transcode can select the matching CRF table"
            )
        ),
    )
...
def check_gpu() -> CheckResult:
    ...
    if not indices:
        return CheckResult(name, "transcode", False, False, detail="nvidia-smi reports no GPUs")
    ...
    return CheckResult(name, "transcode", False, True, detail=f"GPU index {gpu_index}")
    ...
    return [
        ...
        check_binary(
            ...
            install_hint=(
                ...
                "NVIDIA GPU) -- required by track-strip and transcode"
            ),
        ),
        check_binary(
            "ffprobe",
        ...
        check_ffmpeg_encoder(
            ...
            install_hint=(
                ...
                "SVT-AV1 support (transcode's CPU backend, and the only backend that "
        ...
        check_ffmpeg_encoder(
            ...
            required=False,
            install_hint=(
                "this ffmpeg build has no av1_nvenc -- transcode's GPU backend won't "
